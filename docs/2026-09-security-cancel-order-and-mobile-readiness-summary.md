...
Update 20 Sep 2026 (later): the APP SIDE IS DONE TOO and push was VERIFIED ON THE EMULATOR: the development build (`npx expo run:android`) is installed, the app registered its phone token after login and permission, a test push sent straight through `FcmClient` appeared as a notification ("STI PROWARE: Test from PROWARE"), and tapping it opened the Notifications screen. Build notes and the two Windows workarounds (Avast certificate for Java; a space-free `C:\Projectndroid-sdk` junction because the username has spaces) are in the memory file about the mobile app. LIVE END-TO-END TEST PASSED on the user's real phone (realme RMX3999, dev build over USB, 20 Sep 2026): after the phone registered its token, the cashier confirmed a real order's payment on the website and a "Payment Confirmed" push arrived on the phone in the same second (0 waiting and 0 failed jobs). Not yet done (the live test item is now done): a live end-to-end test through a real event (place an order, then have the cashier confirm payment on the website while the queue worker runs); a proper monochrome notification icon (the small icon is a plain circle); an arm64 APK for the user's real phone; and hosting the server online for real students.
Update 21 Sep 2026: the STANDALONE release app (arm64) is installed on the user's realme and works WITHOUT the USB cable: it talks to the laptop over Wi-Fi (`http://192.168.8.42:8001`, a second Laravel started with `php artisan serve --host=0.0.0.0 --port=8001`; the address can be changed on the login screen) and a push sent through Firebase arrived on the phone in 3 to 5 seconds. Needed on the laptop: that server, the queue worker (`php artisan queue:listen --tries=1 --timeout=0`), and Avast Firewall set to trust the Wi-Fi network (the user did this). A release build needs `PROWARE_NATIVE_BUILD_DIR=C:/cx` at `expo prebuild` and the two Windows workarounds in the memory notes. Hosting the server online is deferred by the user.
